    ...
    public class RegTypeService : BaseService<reg_type>, IRegType
    {
        public List<AdogCurRegItem> gCurRegInfos(string deptCode, string doctorCode, string beginDate, string endDate)
        {
            ...
            foreach (var item in list)
            {
                ...
                item.item = db.Ado.SqlQuery<TimeItems>(timequery, new SugarParameter[] { new SugarParameter("@regtid", item.regtid), new SugarParameter("@beginDate", beginDate), new SugarParameter("@endDate", endDate) });
            }
